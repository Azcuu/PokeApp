...
// Modelo
export const PokemonModel = mongodbInstance.model('Pokemons', pokemonSchema, 'Pokemons');

// Funciones
export async function getAllDBPokemon() {
  console.log('Buscando pokemones...');
  const data = await PokemonModel.find().sort({ id: 1 }).lean();
  console.log('Pokemones encontrados:', data.length);
  return data;
}
...
export async function getDBPokemonByType(type) {
  return PokemonModel.find({ type }).lean();
}
